...
dataset GetInput(char *filename, int verbose){
	...
	for (i = 0; i < output.length; ++i){
		...
		if(invalid !=3){
			if(i == output.length-1){ // if it's on the last line, ignore it, it's not an error, it's just a blank line due to excel saving 
				output.length--;
			}
			else{
...
graph LeastSquares(int length, double x[], double y[], double yerr[]){
	...
	result.merr = (sqrt(p/delta));
	result.cerr = (sqrt(s/delta));
	return result;
}
...
int main(int argc, char *argv[]){
	...
	if ((argc > 1 && strcmp(argv[1],"-help") == 0) || argc == 1){  // display help message if either no arguments passed or "-help" passed
		...
		strcpy(filename,argv[1]); // first argument == filename
		if (argc > 2){
			if(strcmp(argv[2],"-v") == 0){ // enable verbosity/**/
				verbose = 1;
			}
		}	
	}
